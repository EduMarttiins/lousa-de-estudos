/* Lousa de Estudos v93 — textos completos de Geografia + runtime estável */
(()=>{
  if(window.__lousaV93Stable)return;
  window.__lousaV93Stable=true;
  const VERSION=93, SEP='§';
  ...
  const GEO_TEXTS={
    cidades:{
      ...
      passage:`<div class="v93StudyLead"><strong>📚 Como estudar:</strong> leia este texto antes das questões. As informações necessárias para responder estão explicadas aqui.</div>
    ...
    desigualdade:{
      ...
      passage:`<div class="v93StudyLead"><strong>📚 Como estudar:</strong> todas as ideias usadas nas questões estão reunidas neste texto.</div>
    ...
    diversidade:{
      ...
      passage:`<div class="v93StudyLead"><strong>📚 Como estudar:</strong> use este texto para revisar a formação cultural brasileira e as desigualdades estudadas no livro.</div>
    ...
    saopaulo:{
      ...
      passage:`<div class="v93StudyLead"><strong>📚 Como estudar:</strong> o texto explica os conceitos e os exemplos culturais usados nas perguntas.</div>
    ...
    jornal:{
      ...
      passage:`<div class="v93StudyLead"><strong>📚 Como estudar:</strong> observe os números e também o que eles significam para a vida das pessoas.</div>
    ...
    tecnologia:{
      ...
      passage:`<div class="v93StudyLead"><strong>📚 Como estudar:</strong> este texto reúne os conceitos necessários para responder às questões sobre tecnologia e trabalho.</div>
  ...
  function applyGradeData(){
    try{
      ...
      const source=selectedGrade==='7'?window.__v93Math7Subject:math5Snapshot;
  ...
  function syncGradeHero(){
    try{
      ...
      document.querySelectorAll('#v93GradePicker .v93GradeBtn').forEach(btn=>{
  ...
  function ensureGradePicker(){
    try{
      ...
      if(!view||!grid)return;
      let host=document.getElementById('v93GradePicker');
      if(!host){
        host=document.createElement('div');
        host.id='v93GradePicker';
        host.className='v93GradePicker';
        host.innerHTML='<div class="v93GradeTitle"><strong>Qual ano vamos revisar?</strong><span>Escolha o ano escolar antes da matéria.</span></div><div class="v93GradeActions"><button type="button" class="v93GradeBtn" data-grade="5">5º ano</button><button type="button" class="v93GradeBtn" data-grade="7">7º ano</button></div>';
        grid.parentNode.insertBefore(host,grid);
        host.querySelectorAll('.v93GradeBtn').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  function gradeStyles(){
    if(document.getElementById('v93GradeStyles'))return;
    const s=document.createElement('style');s.id='v93GradeStyles';
    s.textContent='.v93GradePicker{display:flex;align-items:center;justify-content:space-between;gap:16px;margin:18px 0 24px;padding:16px 18px;border:1px solid var(--lineSoft,#e5e7eb);border-radius:20px;background:#fff;box-shadow:0 8px 24px rgba(15,23,42,.05)}.v93GradeTitle{display:flex;flex-direction:column;gap:4px}.v93GradeTitle strong{font-size:1rem}.v93GradeTitle span{font-size:.86rem;color:#64748b}.v93GradeActions{display:flex;gap:8px;flex-wrap:wrap}.v93GradeBtn{appearance:none;border:1px solid #cbd5e1;background:#fff;color:#334155;border-radius:999px;padding:10px 16px;font-weight:800;font-size:.95rem;cursor:pointer}.v93GradeBtn.active{background:#1d4ed8;color:#fff;border-color:#1d4ed8;box-shadow:0 7px 16px rgba(29,78,216,.18)}@media(max-width:640px){.v93GradePicker{align-items:flex-start;flex-direction:column}.v93GradeActions{width:100%}.v93GradeBtn{flex:1}}';
    document.head.appendChild(s);
  }
  ...
  function installGradeSystem(){
    try{
      ...
      if(typeof renderSubjectCards==='function'&&!renderSubjectCards.__v93Grade){
        ...
        wrapped.__v93Grade=true;renderSubjectCards=wrapped;
      }

      if(typeof applySubjectTheme==='function'&&!applySubjectTheme.__v93Grade){
        const previous=applySubjectTheme;
        ...
        wrapped.__v93Grade=true;applySubjectTheme=wrapped;
      }

      if(typeof resetTheme==='function'&&!resetTheme.__v93Grade){
        ...
        wrapped.__v93Grade=true;resetTheme=wrapped;
      }

      if(typeof updateProgress==='function'&&!updateProgress.__v93Grade){
        ...
        wrapped.__v93Grade=true;updateProgress=wrapped;
      }
  ...
  function refresh(){
    installReligion(); installGeographyTexts(); patchUnlock(); patchTheme(); styles(); gradeStyles(); installGradeSystem(); stamp();
  ...
  setTimeout(()=>{refresh();document.getElementById('v93VersionBootHide')?.remove()},1200);
  window.__lousaV93Ready=true;
})();
